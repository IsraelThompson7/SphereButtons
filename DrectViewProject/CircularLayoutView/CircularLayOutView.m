...
-(void)creatUIToView:(UIView *)view target:(id)target
{
    ...
    cos_a=(360/self.buttonCount)*1.0/180;
...
-(void)moveAnimation:(BOOL)isLeft point:(XYPoint *)point  index:(int)index
{
    ...
    animationTime=time;
      if(isLeft)
      {
          NSTimer *annimationTimer1=[NSTimer scheduledTimerWithTimeInterval:1.0/180 target:self selector:@selector(leftMove:) userInfo:self repeats:YES];
    ...
    else
    {
        ...
        NSTimer *annimationTimer1=[NSTimer scheduledTimerWithTimeInterval:1.0/180 target:self selector:@selector(rightMove:) userInfo:self repeats:YES];
        [annimationTimer1 fire];
...
-(void)leftMove:(NSTimer*)timer
{
    ...
    if(currentButtonCenterX >160   && currentButtonCenterY>130)
    {
        [timer invalidate];
        UIButton *hidenButton=(UIButton *)[currentView viewWithTag:self.buttonCount+100];
...
-(void)rightMove:(NSTimer *)timer
{
    ...
    int currentButtonCenterX=currentButton.center.x;
    int currentButtonCenterY=currentButton.center.x;
        if(currentButtonCenterX==160 &&( (currentButtonCenterY-230)<0))
    {
        [timer invalidate];
        UIButton *hidenButton=(UIButton *)[currentView viewWithTag:self.buttonCount+100];
        hidenButton.hidden=NO;
...
//         NSLog(@"%")
         ...
         double cos_A2=cos_A1-1.0/count;
         if(cos_A2<0)
         {
